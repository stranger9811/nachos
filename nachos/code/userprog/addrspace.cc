...
int AddrSpace::ExtendMemory(int newPages)
{
    newPageTable = new TranslationEntry[newPages + numPages];
    int i;
    for (i = 0; i < numPages; i++) {
        newPageTable[i] = pageTable[i]; 
    }

    for (i = numPages; i < newPages+numPages; i++) {
    ...
    int temp = numPagesAllocated;
	numPagesAllocated += newPages;
