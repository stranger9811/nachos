...
ExceptionHandler(ExceptionType which)
{
	...
	DEBUG('a', "Shutdown, initiated by user program.\n");
       ...
       int bytes = machine->ReadRegister(4);
       int newPages = bytes/PageSize;
       if(bytes%PageSize!=0)
          newPages++;
        ASSERT(newPages + numPagesAllocated <= NumPhysPages);

        int olderNumPages = currentThread->space->ExtendMemory(newPages); 
        
	machine->WriteRegister(2,olderNumPages*PageSize);
      ...
      if(key >= MAX_SEM_COUNT)
        machine->WriteRegister(2,-1);
      else {
          if(exitSemArray[key]== true) {
            semArray[key] = new Semaphore("user semaphore",0);
            exitSemArray[key] = false;
          }
        ...
        if(command == SYNCH_REMOVE) {
          exitSemArray[id] = true;
          delete semArray[id];
        }
        else if (command == SYNCH_GET) {
          int value = semArray[id]->getValue();
          machine->WriteMem(addr,4,value);
        }
        else if (command == SYNCH_SET) {
          int value;
          machine->ReadMem(addr,4,&value);
          if(value>=0)
            semArray[id]->setValue(value);
          else
            machine->WriteRegister(2,-1);
        }
    }
